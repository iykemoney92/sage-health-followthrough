...
type ConversationRow = {
  ...
  tags: string[];
  tone: string;
  category: "radiology" | "insurance" | "bill" | "document";
...
export default function HistoryPage() {
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conversation) => {
      const matchesFilter = activeFilter === "all" || conversation.category === activeFilter;
      const searchable = [
        conversation.title,
  ...
  const activeFilterLabel = historyFilters.find((filter) => filter.value === activeFilter)?.label ?? "All";
    ...
    <ClaritiShell>
      <main className="history-page history-page-v3">
        <section className="history-hero-v3">
          ...
          <div className="history-filter-wrap-v3">
            <button
              type="button"
              className="history-filter-v3"
              aria-haspopup="menu"
              aria-expanded={filterOpen}
              onClick={() => setFilterOpen((open) => !open)}
            >
              <Filter /> <span>{activeFilter === "all" ? "Filter" : activeFilterLabel}</span>
            </button>
            {filterOpen ? (
              <div className="history-filter-menu-v3" role="menu" aria-label="Filter conversations">
            ...
            <>
              {filteredConversations.length > 0 ? (
                <div className="history-list-v3">
                  {filteredConversations.map(({ id, title, question, time, date, tags, tone, icon: Icon }) => (
...
function toConversationRow(session: { id: string; title: string; status: string; updated_at: string; question?: string | null }): ConversationRow {
  ...
  return {
    ...
    category: isRadiology ? "radiology" : isEob ? "insurance" : isBill ? "bill" : "document",
