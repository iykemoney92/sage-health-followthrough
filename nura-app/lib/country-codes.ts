export type CountryCode = { name: string; iso2: string; dialCode: string };
...
export const DEFAULT_COUNTRY = COUNTRY_CODES.find((c) => c.iso2 === "GB")!;

// Longest dial code first so "44" doesn't shadow "1767" etc when parsing a stored number.
const BY_DIAL_CODE_DESC = [...COUNTRY_CODES].sort((a, b) => b.dialCode.length - a.dialCode.length);

export function splitStoredPhone(digits: string): { country: CountryCode; national: string } {
  const clean = digits.replace(/\D/g, "");
  const match = BY_DIAL_CODE_DESC.find((c) => clean.startsWith(c.dialCode));
  if (!match) return { country: DEFAULT_COUNTRY, national: clean };
  return { country: match, national: clean.slice(match.dialCode.length) };
}
